...
frame for evidence capture, and handles automatic severity escalation.
"""
...
class LiveEvent:
    ...
    def __init__(
        ...
        # Base severity based on behavior class
        self.severity: str = self.config.severity_map.get(
    ...
    def update(
        ...
        if (
            ...
            self.status = EventStatus.CONFIRMED.value
            return True  # Signal escalation

        return False

    def close(self, end_frame: int) -> ClassroomEvent:
        """Conclude and package event into an immutable ClassroomEvent."""
        self.is_closed = True
        self.end_frame = max(self.end_frame, end_frame)
        if self.duration_seconds >= 3.0 or self.severity == SeverityLevel.HIGH.value:
            self.status = EventStatus.CONFIRMED.value
